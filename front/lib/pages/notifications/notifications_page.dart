...
import '../../index.dart' show goToFinanceTab, indexTabNotifier;
...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  Future<void> _onTapItem(NotificationModel n) async {
    ...
    if (link != null && link.isNotEmpty && mounted) {
      _navigateForLink(link);
    }
  }

  /// 서버 `linkUrl` 을 해석해 적절한 in-app 위치로 이동시킨다.
  ///
  /// - 등록된 정적 라우트는 그대로 push.
  /// - `/my_wallet` 은 별도 페이지가 아닌 IndexPage 의 Finance 탭으로 이동.
  /// - 인자 기반 동적 라우트(`/chat/room/{id}`, `/sharehouse/{id}` 등)는
  ///   현재 라우트 등록이 없으므로 일단 무시한다 (차후 매핑 가능).
  void _navigateForLink(String linkUrl) {
    const knownStaticRoutes = <String>{
      AppRoutes.index,
      AppRoutes.myContracts,
      AppRoutes.myReviews,
      AppRoutes.savedListings,
      AppRoutes.currentStay,
    };

    if (knownStaticRoutes.contains(linkUrl)) {
      Navigator.of(context).pushNamed(linkUrl);
      return;
    }

    // My Wallet 은 별도 페이지가 없고 IndexPage 의 Finance 탭으로 매핑된다.
    if (linkUrl == '/my_wallet' || linkUrl == '/finance') {
      // 알림 화면을 닫고 홈 인덱스의 Finance 탭으로 이동.
      Navigator.of(context).popUntil((route) => route.isFirst);
      goToFinanceTab();
      return;
    }

    // 커뮤니티 탭으로 보내야 하는 link(예: /community, /chat) — IndexPage 의
    // Community 탭으로 폴백 이동시키면 사용자가 채팅/공지 목록에서 자세한
    // 항목을 찾을 수 있다.
    if (linkUrl.startsWith('/chat') || linkUrl.startsWith('/community')) {
      Navigator.of(context).popUntil((route) => route.isFirst);
      indexTabNotifier.value = 1; // Community 탭
      return;
    }

    // 매물 상세 — 라우트 등록은 없지만 홈 탭으로라도 보내 사용자가 길을 잃지
    // 않도록 한다.
    if (linkUrl.startsWith('/sharehouse')) {
      Navigator.of(context).popUntil((route) => route.isFirst);
      indexTabNotifier.value = 0; // Home 탭
      return;
    }

    // 그 외 — 조용히 무시 (read 처리는 이미 위에서 수행됨)
  }
